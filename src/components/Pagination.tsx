...
export default function Pagination({ page, pageSize, total, meta, onPageChange, onPageSizeChange }: PaginationProps) {
  ...
  const totalRecords = meta?.total ?? total;
  const currentTotal = meta?.current_total ?? Math.min(currentPageSize, totalRecords);
  return (
    <div className="flex flex-col gap-3 border-t border-slate-200 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-sm text-slate-500">
        ...
        <span className="ml-2">
          Showing <span className="font-semibold text-slate-800">{currentTotal}</span> /{' '}
